...
[`page:lsp`](https://monkshark.github.io/page-ide/#internals/lsp/main.md)가 서버와 말하는 프로토콜 층이라면, 이 모듈은 그 위에서 IDE가 실제로 쓰는 오케스트레이션 층이다. 열린 파일을 알맞은 서버로 라우팅하고, 문서 변경을 debounce로 밀어 넣고, 완성 결과를 캐시하고, 서버가 못 주는 결과는 텍스트 스캔으로 메운다.

> English: [main_en.md](https://monkshark.github.io/page-ide/#internals/language/main_en.md)
