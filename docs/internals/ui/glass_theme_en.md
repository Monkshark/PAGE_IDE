...
> `page/ui/src/main/kotlin/page/ui/GlassTokens.kt` · `GlassTheme.kt` — token definitions + Material3 wrapper

Glass is PAGE's aesthetic pillar — the layer that answers "is it pleasant to look at, do you want to open it every day." Color, type, spacing, radius, elevation, and motion are bundled into one set of tokens, and every screen is drawn through them. The swatches below come from the real token values.

> 한국어: [glass_theme.md](https://monkshark.github.io/page-ide/#internals/ui/glass_theme.md)

---

## Palettes

Nine themes. `Signature` is the default identity; the rest cover different moods and light/dark preferences.
...
Wrap content in `GlassTheme` and it provides `LocalGlassTokens` while filling in the Material3 `colorScheme`. The main window and every `DialogWindow` must be wrapped by this function so the same palette applies.
...
Read tokens from any composable via `Glass.colors`, `Glass.type`, `Glass.space`, `Glass.radius`, `Glass.elevation`, `Glass.motion`, and `Glass.palette`.
